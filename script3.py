...
# version for importing and launching as a function
# some comments
...
# }
    ...
    if status_obj == {}:
        ...
        print_txt_on_LCD3(disp, "No status from previous run. Creating a new one.", color="YELLOW", spinner_sec=1)
    ...
    def copy_files_w_status(src_list, target_subfolder, label ="", max_retries = 3, sleep_between_retries = 1):
        ...
        if len(src_list) == 0:
            ...
            print_txt_on_LCD3(disp, f"{label} No files to copy", font_size = 25, color="YELLOW")
            return
...
# if __name__ == "__main__":
    ...
    if len(unmntd) > 0:
        ...
        for dev in unmntd:
            try:
                ...
                print_txt_on_LCD3(disp, f"Mounted {dev} to {mount_point}", font_size=20, color="GREEN")
                time.sleep(1)
    ...
    if all_is_mounted:
        ...
        print_txt_on_LCD3(disp, message, font_size = 16, color="GREEN", spinner_sec = 1)
        time.sleep(1)
    else:
    ...
    try:
        ...
        if len(mp4_files) > 0:
            if (nas_video_dir / target_folder_name).is_dir():
                print(f"Folder {target_folder_name} already exists")
                print_txt_on_LCD3(disp, f"[VIDEOS] Folder {target_folder_name} already exists", font_size = 25, color="YELLOW", spinner_sec= 0.5)
            else:
                print(f"Creating folder {target_folder_name} for videos")
                (nas_video_dir / target_folder_name).mkdir(parents=False, exist_ok=False)
                print_txt_on_LCD3(disp, f"[VIDEOS] Created folder {target_folder_name}", font_size = 25, color="GREEN", spinner_sec= 0.5)
            ...
            print_txt_on_LCD3(disp, f"[VIDEOS] Successfuly Copied {len(mp4_files)} files", font_size = 25, color="GREEN", spinner_sec= 0.5)
